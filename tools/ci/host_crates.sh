...
#     [ci.cargo] host_tools_crate), so the suites run — but the three standalone
#     tools/* crates would be built and linted by nothing, and the wire_lint /
#     telemetry_guard checks would never run as gates (they used to run ONLY
#     from the old hand-written Makefile lint recipe).
